...
# 2. 完全退出 Oh My Pi（不是 /reload——扩展在启动时加载），重新打开
```
...
# 若想恢复被禁用的 SeekAnchor：
#   把 extensions-disabled\deepseek-rl-anchor 移回 extensions\
```
...
独立 OMP 会话、独立空工作目录，各跑一次。空目录 + 固定 cwd 是刻意为之：
前两次尝试被 A/B 跑分的残留文件污染，而残留工作区会让模型走复用路径而非
从零路径，虚高深度（被污染的 C1 设计段 66.9K vs 干净 C1 全程 37.6K）。
...
1. **首轮锚定是唯一强机制。** A vs C1：thinking 多 6 万字符（+160%）。
   两工具表面下模型无法越过 `bash`/`edit` 行动，只能思考。之前把 98K
   归因于 DEEP 引导是错误的；引导贡献在波动内（D1 vs B：+2.2K）。
...
- **thinking 可见性**需要 OMP 配置 `hideThinkingBlock: false`；插件不改
  该设置。
