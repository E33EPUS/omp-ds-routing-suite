...
**E1——互搏因果（每引导 n=2）。** 同一购物车任务，插件默认（weak +
锚定 + 引导），只变尾文本：

| 组 | 尾文本 | 深度 | 互搏 | 密度(/K) | 断言 |
|---|---|---|---|---|---|
| 中性 | （无）| 77.0K / 35.2K | 119 / 27 | 1.55 / 0.77 | 40 / 29 |
| 抑制 | "Do not second-guess…commit." | 0.8K / 26.9K | 0 / 12 | 0 / 0.45 | 26 / 31 |
| 鼓励 | "List candidate approaches, weigh, reject…" | 38.4K / 36.4K | 59 / 33 | 1.54 / 0.91 | 28 / 49 |

均值（中性/抑制/鼓励）：深度 56.1K / 13.9K / 37.4K；互搏 73 / 6 / 46；
断言 34.5 / 28.5 / 38.5。**抑制互搏压垮深度（−75%）与完整度（−17%）、
I will 归零；鼓励显式权衡得到最高互搏密度与最高完整度**——互搏与推理
深度和产出完整度耦合，不是可移除的噪音。注意：每格 n=2；中性组内
波动 ±42K（77.0K vs 35.2K）
...
**E3——resident 目录对照（n=2）。** 购物车任务，插件默认；变量：晋升后
目录 = 全量 vs 窄集（`/dsr-resident on` / settings `{"resident": true}`，
诊断日志证实：`restoreNativeTools resident -> [bash,edit,read,write]`）：

| 会话 | 晋升后目录 | 深度 | 互搏 | 断言 |
|---|---|---|---|---|
| res1 / res2 | 全量（off）| 20.5K / 37.8K | 35 / 32 | 25 / 27 |
| res3 / res4 | 窄集（on）| 13.9K / 73.1K | 4 / 90 | 38 / 38 |

窄集下完整度一致更高（38 vs 25/27，+46%，两组同值）——上游"晋升倒出
完整目录"警告在产出层面有弱支持。深度/互搏波动 ±60K，n=2 不足以结论

**E4——长程相关任务链（论文 P21 设计）。** 解析器任务 8 轮顺序
（write → fix → extend → fix → extend → fix → extend → fix），插件 vs
native 各 n=2：

| 会话 | 条件 | read 类调用 | fix 轮先读 | 断言 |
|---|---|---|---|---|
| e4-a1 / a2 | 插件 | 30% / 44% | 1/3、2/4 | 36 / 35 |
| e4-b1 / b2 | native | 67% / 63% | 3/4、2/3 | 29 / 30 |

方向与论文 P21 一致（引导降低读连续性）：插件组读现有代码更少
（均值 37% vs 65%）。但论文"相关链中引导为负"**未在产出层面复现**——
插件组 8 轮全完成且断言更多（35.5 vs 29.5）。局限：`omp -p` 把 8 条
消息合并为单请求批处理（模型在同一会话内顺序处理 8 任务），非论文的
逐轮交互；批处理下前轮读过的代码已在上下文中，读得少不等于不知道
代码。真实逐轮链（交互式 OMP 会话）不在本次范围内
