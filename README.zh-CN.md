...
**时间成本（社区观感的那一维）。** 全部 12 个 E1 会话的墙钟耗时随互搏
超线性上升：互搏 0 → 29s；2-12 → 36-90s；21-33 → 88-123s；59-119 →
135-220s。互搏换来了深度与完整度，但付的是时间（最高 7.6×）。社区
"互搏多=效果差"的直觉因此是**时间成本直觉，不是质量直觉**：质量
（断言）在每个 E1 样本里都随互搏上升；时间才是真实代价，也是交互
使用中"互搏令人烦躁"的来源
...
**长程互搏趋势（E4-R 二次分析）。** 8 轮链的逐轮互搏密度：插件 a1
1.6→1.5/K、a2 1.8→1.2/K（后半程深度 +60% 而密度下降）；native b1
1.9→1.7/K、b2 1.9→1.0/K（后半程深度崩到 12K）。互搏密度在轮次间
**从不上升**——"长程互搏失控"假说不成立。插件 vs native 的差距在
后半程放大：锚定+引导在长链后期保持甚至加深思考，native 后期变浅——
这正是 E4-R +78% 断言差距的来源（差异主要在后半程轮次）

**F1——J-Space 协议交叉检验（2026-08-16 夜）。** 外部推理时控制套件
（J-Space Cognition Suite V3.6，Apache-2.0，Skill 封装的协议 + 稠密轨
寄存器 ✓/?/✗ 与显式 I/we-need 语义）在同一购物车任务上跑：native+协议
（B×2）与插件+协议（C×2），每会话带套件工作区副本（模型可按需读模块）：

| 会话 | 条件 | 深度 | 互搏 | we/let | 稠密轨标记 | 测试 |
|---|---|---|---|---|---|---|
| b1 / b2 | native + J-Space | 43.9K / 12.2K | 43 / 7 | 1/29、0/11 | 131 / 21 | 过 |
| c1 / c2 | 插件 + J-Space | 39.1K / 43.2K | 51 / 39 | 3/47、0/31 | 118 / 112 | 过 |
| A（3.3）| 仅插件 | 72–98K | 113–119 | — | 0 | 过 |

三个发现：①协议自举有效（模型读 SKILL.md 并执行稠密轨，21–131 个
✓/?/✗ 标记）②稠密轨**不替代**隐式互搏——两者共存（43/51/39 个
hmm/wait 与 112–131 个标记并存）——"结构化重定向"假说不成立，强化
E1 结论（互搏是深度推理固有成分）③语义形态指令**不改变轨迹形态**：
协议的 I/we-need 规则下 we 计数仍 0–3——形态仍由 persona 决定，
是形态锁定结论的独立交叉验证。附注：稠密轨压缩使深度字符下降
（39–43K vs A 的 72–98K）而标记上升——字符深度单独会低估压缩式
推理；稠密轨计数已成为配套指标

**F2——长程叠加（n=2，8 轮解析器链，每轮独立进程）。** 插件 + J-Space
协议（工作区副本 + 账本控制器）vs E4-R 纯插件链：

| 链 | 条件 | 深度 | 互搏 | 断言 |
|---|---|---|---|---|
| jspace-f1 / f2 | 插件 + J-Space | 585.8K / 505.1K | 868 / 724 | 90 / 74 |
| e4r-a1 / a2 | 仅插件 | 255.1K / 396.7K | 395 / 649 | 106 / 102 |

冷启动长程叠加 J-Space = 深度 +67%、断言 −21%（两条链方向一致）——
在我们环境是净开销，与其声称的 Token 效率增益相反。协议自举成功
（loop 门控 + 账本 + 稠密轨都执行了）但不提升产出。范围：每轮独立
...
- **规格明确的任务**（无设计选择）：插件中性——无增益也无害（E2），
  直接给任务即可
- **长程链**（修 → 扩 → 修…）：插件链比 native 多约 78% 已验证断言
  （E4-R）——保持插件开着
- **不要**开 `/dsr-resident on`（实测中性，E3），不要在插件上叠加外部
  协议套件（实测净开销，F2）
- **首轮"想很久"**：那是锚定在工作，不是卡死；第二轮工具全量恢复

**E5——第三任务梯度（n=2，中等设计密度：md2html）。** 三个从零任务
的设计密度梯度：

| 任务 | 设计密度 | 插件深度 | native 深度 | 深度增益 | 断言（插件 vs native）|
|---|---|---|---|---|---|
| 购物车（3.3）| 高 | 72–98K | 32.1K | +160% | 41 vs 25 |
| md2html | 中 | 54.7K（31.9/77.4）| 22.6K（17.5/27.8）| +142% | 23 vs 23 |
| CSV 解析器（E2）| 低 | 29.4K | 30.3K | 0% | 22.5 vs 21.5 |

锚定深度效应在三个任务中的两个上稳健：只要存在设计决策空间（转义
优先级、列表合并、容错），就有 +140% 深度。**深度增益与完成度增益
必须分开报告**：md2html 深度 +142% 但断言零差异（23=23），购物车则
两者都升。组内波动仍大（a1 31.9K vs a2 77.4K）
